...
class LinuxDesktopMonitor {
    static start() {
        ...
        temperatureMonitor.stdout.on("data", (raw_data) => {
            if (raw_data && isNaN(Number(raw_data)) == false) {
                currentDeviceInfo.temperature.temperatureLevel = Number(raw_data);
            }
        });    }
}
class GCloudMonitor {
...
switch (OS.hostname()) {
    ...
    case "client1": {
        currentDeviceInfo.deviceName = "Client Light 1";
        LinuxDesktopMonitor.start();
        break;
    }
    case "client2": {
        currentDeviceInfo.deviceName = "Client Light 2";
        LinuxDesktopMonitor.start();
        break;
    }
